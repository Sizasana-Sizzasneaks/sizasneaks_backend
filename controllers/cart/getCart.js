...
const getCart=async function(req, res, next){
    ...
    try {
        ...
        var retrieveCartByUserIdResult= await retrieveCartByUserId(req.body.userId)
        if(retrieveCartByUserIdResult.ok === true){
            // prepare cart data
            var prepareCartDataResult= await prepareCartData(retrieveCartByUserIdResult.data)
            if(prepareCartDataResult.ok === true){
                var calculateCartSummaryResult = await calculateCartSummary(prepareCartDataResult.data)
